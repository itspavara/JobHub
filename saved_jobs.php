...
<body>
  <h1 class="h2">Save job</h1>

  <?php
  $userid = $_SESSION['User_ID'];

  $sql = "SELECT * FROM save_job s, job_vacancy j WHERE s.job_id = j.job_id AND s.user_id = '$userid'";
  ...
  ?>
      <div class="job-tag">
        ...
        <div class="btn-wrapper-jobs">
          <a class="btn btn-delete" href="saved_jobs.php?id=<?= $row['job_id'] ?>">Delete</a>
        </div>
      </div>
  <?php
    }
  ...
  if (isset($_GET['id'])) {
    ...
    $sql_save = "DELETE FROM save_job WHERE job_id = '$job_id' AND user_id = '$user_id'";
